...
const MATERIAL_STATIC := Color(0.68, 0.75, 0.84, 1.0)
const MATERIAL_DYNAMIC := Color(0.80, 0.73, 0.60, 1.0)
const CONTOUR_STATIC := Color(0.24, 0.62, 0.80, 0.68)
const CONTOUR_DYNAMIC := Color(0.92, 0.60, 0.20, 0.72)
const FOCUS_COLOR := Color(0.66, 0.88, 1.0, 0.78)
...
static func refresh(p1: Node, variant: String, failures: Array[String]) -> void:
	...
	for space in spaces:
		...
		if variant == "state_material":
			_apply_material_state(space, provider, failures)
		elif variant == "state_contour":
			_restore_or_capture_base_material(provider, true)
			_apply_contour_state(space, provider)
		else:
...
static func _apply_material_state(space: LocalMatterSpace, provider: Node3D, failures: Array[String]) -> void:
	...
	material.albedo_color = MATERIAL_STATIC if space.get_provider_kind() == LocalMatterSpace.ProviderKind.STATIC else MATERIAL_DYNAMIC
	material.roughness = roughness
	material.metallic = metallic
	mesh_instance.material_override = material


static func _apply_contour_state(space: LocalMatterSpace, provider: Node3D) -> void:
	...
	material.albedo_color = CONTOUR_STATIC if space.get_provider_kind() == LocalMatterSpace.ProviderKind.STATIC else CONTOUR_DYNAMIC
...
static func _remove_test_nodes(provider: Node3D) -> void:
	for node_name in ["G3SStateContour", "G3SFocusBrackets"]:
